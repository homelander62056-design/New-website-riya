...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const foundProduct = initialProductsData.find((item) => item.id === productId);
    if (foundProduct) {
      setProduct(foundProduct);
    } else {
      setProduct(initialProductsData[0]);
    }
  }, [productId]);

  if (!product) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white text-zinc-500 font-sans">
